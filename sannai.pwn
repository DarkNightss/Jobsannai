...
public OnGameModeInit() {
    ...
    Create3DTextLabel("{3CB371}cong viec tho san {FFFAFA}\n dung lenh {3CB371}[/nhanviec]{FFFAFA} de nhan cong viec nay", -1, 2355.0962, -658.8605, 128.0284, 10.5, 0, 0);
...
public Done_Deer(playerid) {
    ...
    if(Meeter_Kill[playerid] >= 40) {
        bonus = 1000 + random(100);
        money += bonus;
    }
    format(mesaj, sizeof(mesaj), "{1e90ff}(JOB):{FFFFFF} Ban da ban thit huou va nhan duoc {7cfc00}${FFFFFF}%d.", money);
...
public Next_Deer(playerid) {
    ...
    format(mesaj, sizeof(mesaj), "{1e90ff}(JOB):{FFFFFF} Di va giet huou trong vong it nhat la 20m de nhan duoc thit huou. ");
    SendClientMessage(playerid, -1, mesaj);
    switch(rand) {
        case 1: {
            Deer[playerid] = 1;
            ...
            if(hittype == BULLET_HIT_TYPE_OBJECT) {
                if(IsPlayerInRangeOfPoint(playerid, 100.0, 2046.76978, -799.45319, 127.07957) && Shoot_Deer[playerid] == 0) {
                    ...
                    if(Meeter_Kill[playerid] >= 20) {
                        ...
                        format(mesaj, sizeof(mesaj), "{1e90ff}(JOB): {FFFFFF}Ban da giet duoc 1 con nay tu vi tri {1e90ff}%d{FFFFFF}m. hay den cho con huou roi nhan Alt de lay thit. ", Meeter_Kill[playerid]);
                        SendClientMessage(playerid, -1, mesaj);
                        Deep_Deer[playerid] = 1;
                    }else {
                        new mesaj[256];
                        DestroyObject(Hunter_Deer[playerid]);
                        format(mesaj, sizeof(mesaj), "{1e90ff}(JOB): {FFFFFF}Ban da ban truot huou tu vi tri {1e90ff}%d{FFFFFF}m, huou so hai va bo chay. ", Meeter_Kill[playerid]);
                        SendClientMessage(playerid, -1, mesaj);
                        DisablePlayerCheckpoint(playerid);
                        SetTimerEx("Next_Deer", 1000, false, "i", playerid);
                    }
            ...
            if(hittype == BULLET_HIT_TYPE_OBJECT) {
                if(IsPlayerInRangeOfPoint(playerid, 100.0, 2021.1818, -494.0207, 76.1904) && Shoot_Deer[playerid] == 0) {
                    ...
                    if(Meeter_Kill[playerid] >= 20) {
                        ...
                        format(mesaj, sizeof(mesaj), "{1e90ff}(JOB): {FFFFFF} Ban da giet duoc 1 con nay tu vi tri{1e90ff}%d{FFFFFF}m. hay den cho con huou roi an ALT de lay thit. ", Meeter_Kill[playerid]);
                        SendClientMessage(playerid, -1, mesaj);
                        Deep_Deer[playerid] = 1;
                    }else {
                        new mesaj[256];
                        DestroyObject(Hunter_Deer[playerid]);
                        format(mesaj, sizeof(mesaj), "{1e90ff}(JOB): {FFFFFF}Ban da ban truot tu khoang cach {1e90ff}%d{FFFFFF}m, Huou da so va bo chay. ", Meeter_Kill[playerid]);
                        SendClientMessage(playerid, -1, mesaj);
                        DisablePlayerCheckpoint(playerid);
                        SetTimerEx("Next_Deer", 1000, false, "i", playerid);
                    }
            ...
            if(hittype == BULLET_HIT_TYPE_OBJECT) {
                if(IsPlayerInRangeOfPoint(playerid, 100.0, 1632.5769, -599.7444, 62.0889) && Shoot_Deer[playerid] == 0) {
                    ...
                    if(Meeter_Kill[playerid] >= 20) {
                        ...
                        format(mesaj, sizeof(mesaj), "{1e90ff}(JOB): {FFFFFF}Ban da giet chet 1 con huou tu khoang cach {1e90ff}%d{FFFFFF}m. Hay di den va nhan ALT de lay thit huou. ", Meeter_Kill[playerid]);
                        SendClientMessage(playerid, -1, mesaj);
                        Deep_Deer[playerid] = 1;
                    }else {
                        new mesaj[256];
...
public OnPlayerExitVehicle(playerid, vehicleid) {
    ...
    if(vehicleid == Car_Job[playerid]) {
        ...
        Seconds_timer[playerid] = 0;
        SendClientMessage(playerid, ATENTIE, "* Ban có 120 giay de tro ve xe cong viec.");
    }
...
CMD:nhanviec(playerid, params[]) {
    if(!IsPlayerInRangeOfPoint(playerid, 6.0, 2355.0962, -658.8605, 128.0284))  return 1;
    if(JOB[playerid] != 0)  return SendClientMessage(playerid, ATENTIE, "Loi: ban da co cong viec vui long nghi viec[/quitjob]");

    JOB[playerid] = 1;
    inJOB[playerid] = 0;
    SendClientMessage(playerid, -1, "{1E90FF} Ban da nhan viec thanh cong hay lam viec nao[/lamviec]. ");

    return 1;
}
CMD:disannai(playerid, params[])
{
	SetPlayerPos(playerid, 2355.0962,-658.8605,128.0284);
    ...
    if(JOB[playerid] == 1) {
        if(!IsPlayerInRangeOfPoint(playerid, 1.0, 2351.7666, -658.1649, 128.0146)) {
            if(inJOB[playerid] == 1) return 1;
                ...
                SendClientMessage(playerid, ATENTIE, "* Ban phai o noi lam viec hay theo doi checkpoint tren ban do ");
                SetPlayerCheckpoint(playerid, 2351.7666, -658.1649, 128.0146, 3.5);
            }
        }else {
            if(inJOB[playerid] == 1)
                 SendClientMessage(playerid, ATENTIE, "Loi:Ban dang lam viec ");
            else {
                inJOB[playerid] = 1;
                new rand = random(3);
                switch(rand) {
